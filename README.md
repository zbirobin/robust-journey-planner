...
To run our code, you need 8GB of memory.

Using our journey planning interface is straightforward, just follow the instructions in the notebook called *main.ipynb*. First, you need to be aware that loading the data could take some time but no more than 1 or 2 minutes. Moreover, complex queries need to explore a tremendous number of routes, since we are asked to output all the possible routes between two stops. Therefore the time for a given query could be huge. However, simple queries should return a list of routes in reasonable time (less than 2 minutes). The default values should give you an nice overview!
